    ...
    def findLadders(self, start, end, dict):
        ...
        while queue:
            ...
            for i in xrange(length_0):  #O(V)
                current = queue[i]
                if current.string==end:  # reached
                    self.append(current, result)
            if result:  # shortest, thus pruning
                return result
...
class Solution:  # use set to mimic queue, fastest
    def findLadders(self, start, end, dict):
        ...
        while end not in candidates[current]:
            ...
            if len(candidates[current])==0: return []  # path impossible
