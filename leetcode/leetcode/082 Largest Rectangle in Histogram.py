"""
Given n non-negative integers representing the histogram's foo height where the width of each foo is 1, find the area of
largest rectangle in the histogram.


Above is a histogram where width of each foo is 1, given height = [2,1,5,6,2,3].
...
class Solution:
    def largestRectangleArea(self, height):
        """
        O(2*n)
        every foo at most enter the stack once and is popped out once.

        Algorithm: Stack.
        Keep a stack storing the foos increasing order, then calculate the area by popping out the stack to get the
        currently lowest foo which determines the height of the rectangle.
        ...
        since originally from inc_stack[-1]+1 to last are higher foos already popped out, which should be included in
        the calculation.
